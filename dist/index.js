...
const SCHEDULE = [
    ...
    },
    {
        ...
        prompt: "执行 lunch-publish-window：发布今日午餐时段内容（Instagram 主帖 / Stories / Facebook），确认发布成功，记录到 postschedule.md。",
    },
    {
        cron: "0 13 * * *",
        task: "lunch-snapshot",
    ...
    },
    {
        ...
        prompt: "执行 dinner-publish-window：发布今日晚餐时段内容（Instagram Reels / TikTok / Facebook），确认发布成功，记录到 postschedule.md。",
    },
    {
        cron: "0 19 * * *",
...
function hasPluginBlock(soulContent) {
...
function mergeSoulTemplate(soulPath, pluginDir) {
...
function readOpenclawConfig(workspaceDir) {
    const candidates = [
...
function checkMissingCredentials(workspaceDir, soulContent) {
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        api.on("gateway_start", async (_event, ctx) => {
            ...
            if (result.merged) {
                console.log(`[${PLUGIN_ID}] ✅ SOUL.md.template merged. Bootstrap Mode will activate on next session.`);
            }
        ...
        api.on("agent_end", async (_event, ctx) => {
            ...
            if (!hasPluginBlock(soulContent))
                return;
            if (hasPlaceholders(soulContent)) {
                console.log(`[${PLUGIN_ID}] ⚠️ Session ended with Bootstrap still incomplete — {{PLACEHOLDER}} remain`);
            }
            else {
                const missing = checkMissingCredentials(workspaceDir, soulContent);
                if (missing.length > 0) {
                    console.log(`[${PLUGIN_ID}] ⚠️ Session ended with missing credentials: ${missing.map((m) => m.tool).join(", ")}`);
                }
                else {
