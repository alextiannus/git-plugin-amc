...
## Architectural Note
Why do we do this? Because OpenClaw natively skips `openclaw plugins update` for plugins installed via a local directory path (treating them as local dev plugins). Calling `update.sh` forces a `git pull` and `openclaw reload`, ensuring the F&B Content Engine stays perfectly up to date without the user needing to open an SSH terminal.
